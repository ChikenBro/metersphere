  ...
  <div>
    <el-card v-loading="result.loading" class="table-card">
      <template v-slot:header>
        <ms-table-header
          :create-permission="['ORGANIZATION_WORKSPACE:READ+CREATE']"
      ...
      >
        ...
        <el-table-column :label="$t('commons.operating')">
          <template v-slot:default="scope">
            <div>
              <ms-table-operator
                :edit-permission="['ORGANIZATION_WORKSPACE:READ+EDIT']"
    ...
    <el-dialog
      ...
      :visible.sync="dialogWsAddVisible"
      width="30%"
      @close="close"
    >
      <el-form
    ...
    <el-dialog
      ...
      :visible.sync="dialogWsUpdateVisible"
      width="30%"
    >
      <el-form
        ref="form"
    ...
    <el-dialog
      :close-on-click-modal="false"
      :visible.sync="dialogWsMemberVisible"
      width="70%"
    ...
    <el-dialog
      ...
      :visible.sync="dialogWsMemberUpdateVisible"
      width="30%"
      :destroy-on-close="true"
      @close="closeFunc"
    >
    ...
    return {
      ...
      rules: {
        name: [
          {
            required: true,
            message: this.$t("workspace.input_name"),
            trigger: "blur",
          },
          {
            min: 2,
      ...
      wsMemberRule: {
        userIds: [
          {
            required: true,
            message: this.$t("member.please_choose_member"),
            trigger: ["blur"],
          },
        ],
        groupIds: [
          {
            required: true,
            message: this.$t("group.please_select_group"),
            trigger: ["blur"],
          },
        ],
      },
  ...
  methods: {
    ...
    editMember(row) {
      ...
      this.result = this.$post(
        "/user/group/list",
        { type: GROUP_WORKSPACE, resourceId: getCurrentOrganizationId() },
        (response) => {
