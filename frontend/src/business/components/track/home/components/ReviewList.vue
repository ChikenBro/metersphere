<template>
  ...
  >
    ...
    >
      ...
      <el-table-column :label="$t('test_track.review.result_distribution')">
        <template v-slot:default="scope">
          <el-tooltip
            :content="
              getResultTip(scope.row.total, scope.row.reviewed, scope.row.pass)
            "
            placement="top"
            :enterable="false"
            class="item"
  ...
  methods: {
    initTableData(type) {
      ...
      this.result = this.$post(
        "/test/case/review/list/all/relate",
        param,
        (response) => {
