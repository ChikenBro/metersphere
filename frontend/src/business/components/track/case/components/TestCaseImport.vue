<template>
  <el-dialog
    ...
    :title="$t('test_track.case.import.case_import')"
    :visible.sync="dialogVisible"
    @close="close"
  >
    <el-tabs v-model="activeName" simple @tab-click="clickTabs">
      <el-tab-pane
        :label="$t('test_track.case.import.excel_title')"
        name="excelImport"
      >
        ...
        <el-row class="import-row">
          ...
          <label class="ms-license-label">{{
            $t("test_track.case.import.import_desc")
          }}</label>
        </el-row>
        <el-row class="import-row">
          ...
          >
            ...
            <template v-slot:tip>
              <div class="el-upload__tip">
                {{ $t("test_track.case.import.upload_limit") }}
              </div>
            </template>
          </el-upload>
        </el-row>
        ...
        <el-row v-if="showExcelImportContinueBtn" style="text-align: right">
          <div style="margin-right: 20px; margin-bottom: 10px">
            <el-checkbox v-model="uploadIgnoreError">{{
              $t("test_track.case.import.ignore_error")
            }}</el-checkbox>
          </div>
          <el-button type="primary" @click="uploadContinue(false)"
      ...
      <!-- Xmind 导入 -->
      <el-tab-pane
        :label="$t('test_track.case.import.xmind_title')"
        name="xmindImport"
        style="border: 0px"
      >
        <el-row class="import-row" style="margin-left: 34px">
        ...
        <el-row class="import-row">
          <el-upload
            ...
            action=""
            :on-exceed="handleExceed"
            :before-upload="uploadValidateXmind"
            :on-error="handleError"
            :show-file-list="false"
          ...
          >
            ...
            <template v-slot:tip>
              <div class="el-upload__tip">
                {{ $t("test_track.case.import.upload_xmind") }}
              </div>
            </template>
          </el-upload>
        </el-row>
        ...
        <el-row v-if="showXmindImportContinueBtn" style="text-align: right">
          <div style="margin-right: 20px; margin-bottom: 10px">
            <el-checkbox v-model="uploadXmindIgnoreError">{{
              $t("test_track.case.import.ignore_error")
            }}</el-checkbox>
          </div>
          <el-button type="primary" @click="uploadContinue(true)"
            >{{ $t("test_track.case.import.continue_upload") }}
          </el-button>
          <el-button @click="close">{{ $t("commons.cancel") }}</el-button>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    downloadTemplate() {
      this.$fileDownload(
        "/test/case/export/template/" + this.projectId + "/" + this.importType
      );
    },
    downloadXmindTemplate() {
      axios
        .get(
          "/test/case/export/xmindTemplate/" +
            this.projectId +
            "/" +
            this.importType,
          { responseType: "blob" }
