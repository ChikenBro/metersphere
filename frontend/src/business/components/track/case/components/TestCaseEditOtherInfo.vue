  ...
  >
    ...
    <el-tab-pane :label="$t('test_track.case.relate_test')" name="relateTest">
      <el-col v-if="form.list" :span="7" :offset="1">
        <span class="cast_label"
          >{{ $t("test_track.case.relate_test") }}：</span
        >
      ...
      <el-col v-else :span="7" style="margin-top: 10px">
        <el-form-item :label="$t('test_track.case.relate_test')">
          <el-cascader
            ...
            :options="sysList"
            filterable
            :placeholder="$t('test_track.case.please_select_relate_test')"
            show-all-levels
            :props="props"
            :disabled="readOnly"
          ></el-cascader>
    ...
    <el-tab-pane :label="$t('test_track.related_requirements')" name="demand">
      <el-col :span="7">
        <el-form-item
          :label="$t('test_track.related_requirements')"
        ...
        >
          <el-select
            ...
            filterable
            :disabled="readOnly"
            :placeholder="$t('test_track.please_related_requirements')"
            class="ms-case-input"
      ...
      <el-col :span="7">
        <el-form-item
          ...
          :label="$t('test_track.case.demand_name_id')"
          :label-width="labelWidth"
          prop="demandName"
        >
          <el-input v-model="form.demandName" :disabled="readOnly"></el-input>
        </el-form-item>
      </el-col>
    </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    _handleDelete(file, index) {
      ...
      let i = this.uploadList.findIndex(
    ...
    getFileMetaData(id) {
      ...
      if (testCaseId) {
        this.result = this.$get(
          "test/case/file/metadata/" + testCaseId,
          (response) => {
    ...
    getDemandOptions() {
      if (this.demandOptions.length === 0) {
        ...
        this.$get("demand/list/" + this.projectId)
          .then((response) => {
            this.demandOptions = response.data.data;
            this.demandOptions.unshift({
              id: "other",
              name: this.$t("test_track.case.other"),
              platform: "Other",
            });
