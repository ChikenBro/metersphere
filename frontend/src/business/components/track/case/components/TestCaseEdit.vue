        ...
        >
          ...
          >
            <ms-code-edit
              ...
              height="400px"
              :read-only="readOnly"
              :data.sync="form.stepDescription"
              :modes="modes"
            />
          </el-form-item>

          <test-case-step-item
            v-if="form.stepModel === 'STEP'"
            :label-width="formLabelWidth"
            :form="form"
            :read-only="readOnly"
          />
          ...
          <el-row v-if="type != 'add'" style="margin-top: 10px">
...
import {
  ...
  TEST,
  TEST_CASE,
} from "@/business/components/api/definition/model/JsonData";
  ...
  methods: {
    ...
    getTestOptions(val) {
      ...
      if (this.form.type === "performance") {
        ...
        return new Promise((resolve, reject) => {
          this.$get(
            "/api/automation/module/list/" + this.projectId,
            (response) => {
    ...
    buildTreeValueApiCase(list) {
      list.forEach((item) => {
        (item.value = item.id), (item.label = item.name), (item.leaf = true);
        if (item.children) {
    ...
    initAddFuc() {
      ...
      if (this.type === "add") {
        ...
        parseCustomField(
          ...
          buildTestCaseOldFields(this.form)
        );
    ...
    open(testCase) {
      ...
      getTemplate("field/template/case/get/relate/", this).then((template) => {
        this.testCaseTemplate = template;
        initFuc(testCase);
      });
    },
