<template>
  <div>
    ...
    <el-button
      ...
      size="mini"
      @click="relateIssue"
      >{{ $t("test_track.case.relate_issue") }}</el-button
    >
    <el-tooltip
      v-permission="['PROJECT_TRACK_PLAN:READ+RELEVANCE_OR_CANCEL']"
      class="item"
    ...
    >
      ...
      <ms-table-column :label="$t('test_track.issue.title')" prop="title">
      </ms-table-column>
      ...
      <ms-table-column :label="$t('test_track.issue.platform')" prop="platform">
      </ms-table-column>
      ...
      <el-table-column :label="$t('test_track.issue.operate')">
        <template v-slot:default="scope">
          <el-tooltip
            :content="$t('test_track.issue.close')"
            placement="top"
            :enterable="false"
          >
            <el-button
              ...
              size="mini"
              circle
              :disabled="scope.row.platform !== 'Local'"
              @click="closeIssue(scope.row)"
            />
          </el-tooltip>
          <el-tooltip
            :content="$t('test_track.case.unlink')"
            placement="top"
            :enterable="false"
          >
            <el-button
...
export default {
  ...
  created() {
    getIssueTemplate().then((template) => {
  ...
  methods: {
    ...
    deleteIssue(row) {
      this.page.result = this.$post(
        "/issues/delete/relate",
        { id: row.id, caseId: this.caseId },
        () => {
          this.getIssues();
