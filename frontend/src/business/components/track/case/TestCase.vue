  ...
  <ms-container v-if="renderComponent" v-loading="loading">
    ...
    <ms-main-container>
      <el-tabs v-model="activeName" @tab-click="addTab" @tab-remove="removeTab">
        ...
        >
          <test-case-list
            :check-redirect-i-d="checkRedirectID"
            :is-redirect-edit="isRedirectEdit"
        ...
        <el-tab-pane
          name="default"
          :label="$t('api_test.definition.case_title')"
        >
          <ms-tab-button
        ...
        >
          <div class="ms-api-scenario-div">
            <test-case-edit
              :current-test-case-info="item.testCaseInfo"
        ...
        <el-tab-pane
          v-if="hasPermission('PROJECT_TRACK_CASE:READ+CREATE')"
          name="add"
        >
...
export default {
  ...
  methods: {
    ...
    addTab(tab) {
      ...
      if (tab.name === "add") {
        ...
        this.tabs.push({
          label: label,
          name: name,
          testCaseInfo: { testCaseModuleId: "", id: getUUID() },
        });
      }
      if (tab.name === "edit") {
        let label = this.$t("test_track.case.create");
    ...
    init(route) {
      let path = route.path;
      if (
        path.indexOf("/track/case/edit") >= 0 ||
        path.indexOf("/track/case/create") >= 0
      ) {
