      ...
      <el-container class="main-content">
        ...
        <el-container>
          <el-main class="case-content">
            ...
            <div v-if="!lineStatus" style="text-align: center">
...
*/
    ...
    getReviews(flag) {
      ...
      if (this.projectId) {
        ...
        this.result = this.$post(
          this.buildPagePath("/test/case/reviews/case"),
          this.condition,
          (response) => {
            ...
            this.lineStatus =
              tableData.length === 50 && this.testReviews.length < this.total;
          }
        );
      }
    },
    ...
    getProject() {
      if (this.reviewId) {
        this.$post(
          ...
          { userId: getCurrentUserId(), workspaceId: getCurrentWorkspaceId() },
          (res) => {
            let data = res.data;
            if (data) {
              this.projects = data;
              const index = data.findIndex(
                (d) => d.id === getCurrentProjectID()
              );
              if (index !== -1) {
    ...
    getProjectNode(projectId) {
      ...
      this.result = this.$post(
        "/case/node/list/all/review",
        { reviewId: this.reviewId, projectId: this.projectId },
        (response) => {
          this.treeNodes = response.data;
        }
      );
