<template>
  <el-card v-loading="cardResult.loading" class="table-card">
    ...
    >
      ...
      <el-table-column min-width="180" :label="$t('commons.operating')">
        ...
        <template v-slot:default="scope">
          <div>
            <ms-table-operator
              :edit-permission="['PROJECT_TRACK_PLAN:READ+EDIT']"
            ...
            >
              <template v-slot:middle>
                ...
                <ms-table-operator-button
                  v-if="scope.row.reportId"
                  v-permission="['PROJECT_TRACK_PLAN:READ+EDIT']"
                  :tip="$t('test_track.plan_view.view_report')"
                  icon="el-icon-s-data"
  ...
  methods: {
    ...
    initTableData() {
      ...
      this.cardResult = this.$post(
        this.buildPagePath(this.queryPath),
        this.condition,
        (response) => {
