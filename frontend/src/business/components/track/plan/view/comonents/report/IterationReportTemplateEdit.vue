      ...
      <div class="container">
        <el-aside>
          <div class="description">
            <span class="title">{{
              $t("test_track.plan_view.component_library")
            }}</span>
          ...
          >
            <transition-group>
              <template-component-bar
                v-for="item in components"
                :key="item"
                :component="componentMap.get(item)"
              />
            </transition-group>
          </draggable>
        </el-aside>
...
import NewTemplateComponent from "./TemplateComponent/NewTemplateComponent";

export default {
  name: "IterationReportTemplateEdit",
  components: {
  ...
  methods: {
    ...
    initComponents() {
      this.componentMap.forEach((value, key) => {
        if (
          this.template.content.components.indexOf(key) < 0 &&
          this.components.indexOf(key) < 0
        ) {
      ...
      this.template.content.components.forEach((item) => {
        ...
        if (preview && preview.type != "custom") {
          ...
          if (this.template.content.customComponent) {
            let customComponent = this.template.content.customComponent.get(
              item.toString()
            );
            if (customComponent) {
              this.previews.push({
    ...
    handleSave() {
      ...
      if (this.template.name == null || this.template.name == "") {
        ...
        this.$warning(
          this.$t("test_track.plan_view.template_special_characters")
        );
        return;
      }
