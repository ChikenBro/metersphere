<template>
  <div class="card-container">
    <el-card v-loading="result.loading" class="card-content">
        ...
        <span v-for="item in fields" :key="item.key">
          ...
          >
            <template v-slot:default="{ row }">
              ...
              <el-tag
                v-else-if="row.caseStatus === 'success'"
                size="mini"
                type="success"
              >
                {{ row.caseStatus }}
              </el-tag>
              <el-tag v-else-if="row.caseStatus === 'run'" size="mini">
                {{ row.caseStatus }}
              </el-tag>
              <span v-else>-</span>
            </template>
    ...
    <ms-plan-run-mode ref="runMode" @handleRunBatch="runBatch" />
  ...
  methods: {
    ...
    initTable() {
      ...
      if (this.planId) {
        this.condition.testPlanId = this.planId;
        this.$post(
          "/test/plan/load/case/list/" + this.currentPage + "/" + this.pageSize,
          this.condition,
          (response) => {
    ...
    updateStatus(loadCase, status) {
      this.$post(
        "/test/plan/load/case/update",
        { id: loadCase.id, status: status },
        () => {
          this.$post(
            "/test/plan/edit/status/" + loadCase.testPlanId,
            {},
            () => {
              this.initTable();
            }
    ...
    handleDelete(loadCase) {
      this.result = this.$get(
        "/test/plan/load/case/delete/" + loadCase.id,
        () => {
    ...
    getReport(data) {
      ...
      this.$post(
        "/test/plan/load/case/report/exist",
        {
