...
export default {
  ...
  methods: {
    ...
    getReport() {
      if (this.reportId) {
        this.result = this.$get(
          "/case/report/get/" + this.reportId,
          (response) => {
            this.report = response.data;
            this.report.content = JSON.parse(response.data.content);
            if (this.report.content.customComponent) {
              this.report.content.customComponent = jsonToMap(
                this.report.content.customComponent
              );
            }
            this.getMetric();
            this.initPreviews();
    ...
    getMetric() {
      this.result = this.$get(
        "/test/plan/get/statistics/metric/" + this.planId,
        (response) => {
