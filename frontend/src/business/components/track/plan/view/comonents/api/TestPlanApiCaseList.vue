<template>
  <div class="card-container">
    <el-card v-loading="result.loading" class="card-content">
      ...
      <test-plan-api-case-result ref="apiCaseResult" :response="response" />
      ...
      <ms-plan-run-mode ref="runMode" @handleRunBatch="handleRunBatch" />
    </el-card>
...
import {
  ...
  CASE_PRIORITY,
  RESULT_MAP,
} from "../../../../../api/definition/model/JsonData";
...
export default {
  ...
  methods: {
    ...
    initTable() {
      ...
      if (this.reviewId) {
        ...
        this.result = this.$post(
          "/test/case/review/api/case/list/" +
            this.currentPage +
            "/" +
            this.pageSize,
          this.condition,
      ...
      if (this.planId) {
        ...
        this.result = this.$post(
          "/test/plan/api/case/list/" + this.currentPage + "/" + this.pageSize,
          this.condition,
          (response) => {
            this.total = response.data.itemCount;
            this.tableData = response.data.listObject;
          ...
          callback: (action) => {
            if (action === "confirm") {
              ...
              if (this.reviewId) {
                ...
                this.$post(
                  "/test/case/review/api/case/batch/delete",
                  param,
                  () => {
                    if (this.$refs.table) {
                      this.$refs.table.clear();
    ...
    getData() {
      return new Promise((resolve) => {
        ...
        if (this.condition != null && this.condition.selectAll) {
          ...
          this.$post(
            "/test/plan/api/case/selectAllTableRows",
            selectAllRowParams,
            (response) => {
    ...
    getReportResult(apiCase) {
      let url =
        "/api/definition/report/getReport/" + apiCase.id + "/" + "API_PLAN";
