        ...
        <el-row>
          <el-col :span="8" :offset="16">
            <el-input
              ...
              size="small"
              @blur="getTestCases"
              @keyup.enter.native="getTestCases"
            />
          </el-col>
        </el-row>
      </template>
  ...
  methods: {
    ...
    saveCaseRelevance() {
      ...
      if (this.reviewId) {
        ...
        this.result = this.$post(
          "/test/review/load/case/relevance",
          param,
          () => {
            this.selectIds.clear();
            this.$success(this.$t("commons.save_success"));
    ...
    getTestCases() {
      if (this.planId) {
        ...
        this.condition.projectId = this.projectId;
        this.result = this.$post(
          this.buildPagePath("/test/plan/load/case/relevance/list"),
          this.condition,
          (response) => {
      ...
      if (this.reviewId) {
        ...
        if (this.projectId) {
          this.condition.projectId = this.projectId;
          this.result = this.$post(
            this.buildPagePath("/test/review/load/case/relevance/list"),
            this.condition,
            (response) => {
              let data = response.data;
              this.total = data.itemCount;
