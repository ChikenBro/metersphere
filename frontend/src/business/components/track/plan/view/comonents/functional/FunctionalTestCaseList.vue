    ...
    >
      ...
      <template v-slot:button>
        <ms-table-button
          ...
          :content="$t('test_track.plan_view.my_case')"
          @click="searchMyTestCase"
        />
        <ms-table-button
          v-if="showMyTestCase"
          v-permission="['PROJECT_TRACK_CASE:READ']"
          icon="el-icon-files"
          :content="$t('test_track.plan_view.all_case')"
          @click="searchMyTestCase"
        />
        <ms-table-button
    ...
    >
      ...
      <span v-for="item in fields" :key="item.key">
        ...
        >
          <template v-slot:default="scope">
            <ms-tag
              v-for="(tag, index) in scope.row.showTags"
              :key="tag + '_' + index"
              type="success"
              effect="plain"
              :content="tag"
              ...
              <el-dropdown class="test-case-status" @command="statusChange">
                ...
                <el-dropdown-menu slot="dropdown" chang>
                  <el-dropdown-item
                    ...
                    :command="{ id: scope.row.id, status: 'Pass' }"
                  >
                  ...
                  <el-dropdown-item
                    :disabled="!hasEditPermission"
                    :command="{ id: scope.row.id, status: 'Skip' }"
                  >
...
export default {
  ...
  methods: {
    ...
    initTableData() {
      ...
      if (this.planId) {
        this.result = this.$post(
          this.buildPagePath("/test/plan/case/list"),
          this.condition,
          (response) => {
            ...
            for (let i = 0; i < this.tableData.length; i++) {
              if (this.tableData[i]) {
                if (this.tableData[i].customFields) {
                  this.tableData[i].customFields = JSON.parse(
                    this.tableData[i].customFields
                  );
                }
                this.$set(
                  this.tableData[i],
                  "showTags",
                  JSON.parse(this.tableData[i].tags)
                );
                this.$set(
                  this.tableData[i],
                  "issuesSize",
                  this.tableData[i].issuesCount
                );
                this.$set(
                  this.tableData[i],
                  "issuesContent",
                  JSON.parse(this.tableData[i].issues)
                );
              }
            }
    ...
    _handleDelete(testCase) {
      ...
      this.result = this.$post(
        "/test/plan/case/delete/" + testCaseId,
        {},
        () => {
          this.$emit("refresh");
          this.$success(this.$t("test_track.cancel_relevance_success"));
